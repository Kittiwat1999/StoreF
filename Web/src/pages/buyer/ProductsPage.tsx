    ...
    return (
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-3">
        ...
        <div className="relative flex items-center gap-2 rounded-3xl border border-slate-200 bg-white p-3">
          ...
          >
            <span>{sortOptions.find((option) => option.value === sortOrder)?.label ?? "Sort"}</span>
            ...
            <div className="absolute right-0 top-full z-10 mt-2 w-56 overflow-hidden rounded-3xl border border-slate-200 bg-white text-sm shadow-lg">
              {sortOptions.map((option) => (
                ...
                >
                  ...
                  {sortOrder === option.value && <span className="text-orange-500">✓</span>}
                </button>
              ))}
            </div>
          )}
  ...
  return (
    <main className="mx-auto max-w-screen-2xl px-4 py-8 sm:px-6">
      <div className="mb-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-500">
              Browse available items and use the buttons on each card to purchase now or add them to your cart.
            </p>
          </div>

          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-2xl bg-white px-3 py-2 text-sm shadow-sm md:hidden"
        ...
        {/* mobile: collapsible filters (visible when toggled) */}
        {filtersOpen && (
          <div id="products-filters" className="mt-4 md:hidden">
            <ProductsFilters />
          </div>
        )}

        {/* md+ show filters by default */}
        <div id="products-filters" className="hidden md:block mt-4">
          <ProductsFilters />
        </div>
      </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
          <h3 className="text-lg font-semibold text-slate-900">No products match your filters</h3>
          <p className="mt-2 text-sm text-slate-600">Try clearing the price filters or changing the sorting option.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
          {filteredProducts.map((product) => (
            <ProductsCard key={product.id} product={product} onViewDetails={() => navigate(`/products/${product.id}`)} />
          ))}
        </div>
      )}
    </main>
